import { ArrowLeft, CalendarCheck, Clock } from 'lucide-react';
import { Link, useParams } from 'react-router';

import { Button } from '@/components/ui/button';
import { AddToQuoteButton } from '@/quote/AddToQuoteButton';
import { ProductsGrid } from '@/shop/components/ProductsGrid';
import { useCatalog, useCatalogItem } from '@/shop/hooks/use-catalog';
import { formatLps } from '@/shop/lib/format';
import { serviceImage } from '@/shop/lib/service-image';

export const ProductPage = () => {
    const { idSlug } = useParams();
    const { data: item, isLoading, isError } = useCatalogItem(idSlug);

    const { data: related } = useCatalog({ limit: 8 });
    const relatedItems = (related?.products ?? []).filter(
        (i) => i.id !== item?.id && i.category === item?.category,
    );

    if (isLoading) {
        return (
            <div className="container mx-auto px-4 py-24 text-center text-muted-foreground lg:px-8">
                Cargando diseño…
            </div>
        );
    }

    if (isError || !item) {
        return (
            <div className="container mx-auto px-4 py-24 text-center lg:px-8">
                <p className="text-muted-foreground">No encontramos ese diseño.</p>
                <Button variant="outline" render={<Link to="/shop" />} className="mt-4">
                    Volver al catálogo
                </Button>
            </div>
        );
    }

    return (
        <div className="container mx-auto px-4 py-10 lg:px-8">
            <Link
                to="/shop"
                className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground"
            >
                <ArrowLeft className="h-4 w-4" />
                Todos los servicios
            </Link>

            <div className="mt-6 grid gap-10 md:grid-cols-2">
                <div className="overflow-hidden rounded-2xl bg-muted">
                    <img
                        src={serviceImage(item.image, item.category)}
                        alt={item.name}
                        className="aspect-square w-full object-cover"
                    />
                </div>

                <div className="flex flex-col">
                    <p className="text-xs uppercase tracking-wide text-muted-foreground">
                        {item.category}
                    </p>
                    <h1 className="mt-2 font-montserrat text-3xl tracking-tight">
                        {item.name}
                    </h1>

                    <div className="mt-4 flex items-center gap-6">
                        <span className="text-2xl font-semibold">
                            {formatLps(item.price)}
                        </span>
                        <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground">
                            <Clock className="h-4 w-4" />
                            {item.durationMin} min
                        </span>
                    </div>

                    {item.description && (
                        <p className="mt-6 text-muted-foreground">{item.description}</p>
                    )}

                    <div className="mt-auto flex flex-wrap gap-3 pt-8">
                        <Button
                            size="lg"
                            className="h-11 px-6"
                            render={
                                <Link to={`/shop/agendar?serviceId=${item.serviceId}`} />
                            }
                        >
                            <CalendarCheck className="h-4 w-4" />
                            Agendar este servicio
                        </Button>
                        <AddToQuoteButton item={item} size="lg" className="h-11 px-6" />
                    </div>
                </div>
            </div>

            {relatedItems.length > 0 && (
                <div className="mt-20">
                    <h2 className="mb-8 font-montserrat text-2xl tracking-tight">
                        Diseños relacionados
                    </h2>
                    <ProductsGrid items={relatedItems} />
                </div>
            )}
        </div>
    );
};
